      ...
      >
        <div className="group cursor-pointer w-full h-full p-8 rounded-[14px] relative bg-[#251B4B] backdrop-blur-lg transition-transform duration-300 hover:scale-[1.02] overflow-hidden">
          ...
          <div className="shine-container aspect-[4/5] bg-gray-700 rounded-xl z-10 relative overflow-hidden before:absolute before:content-[''] before:inset-0 before:w-1/2 before:h-[200%] before:bg-gradient-to-r before:from-transparent before:via-white/30 before:to-transparent">
            <Image src={`/images/members/${data.image}`} alt={data.name} fill className="object-cover w-full h-full" />
          </div>
...
function MemberDialog({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/60 z-40 backdrop-blur-md transition-opacity duration-500" style={{ opacity: isAnimating ? 1 : 0 }} />
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <Dialog.Content
            style={ !isAnimating ? { top: cardBounds.top, left: cardBounds.left, width: cardBounds.width, height: cardBounds.height } : {} }
            className={`fixed shadow-2xl focus:outline-none transition-all duration-500 ease-in-out overflow-hidden ${ isAnimating ? 'top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg h-[95vh] rounded-2xl bg-[#251B4B]/80' : 'rounded-[14px]' }`}
          >
            <div className="absolute inset-0 -z-10">
                <AnimatedStarfield />
                <AuroraBorealis />
            ...
            <div className={`pointer-events-none absolute inset-0 transition-opacity duration-300 delay-200 ${isAnimating ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className={`h-full w-full flex flex-col transition-opacity duration-300 delay-200 ${isAnimating ? 'opacity-100' : 'opacity-0'}`}>
              <div className="flex-grow overflow-y-auto p-10 custom-scrollbar">
                ...
                <div className="w-full aspect-[4/5] rounded-xl overflow-hidden mb-6 relative ring-2 ring-offset-2 ring-offset-[#251B4B] ring-purple-500/50">
                  <Image src={`/images/members/${data.image}`} alt={data.name} fill className="object-cover" />
                </div>
                <h2 className="text-4xl font-bold font-nexa text-transparent bg-clip-text bg-gradient-to-r from-sky-300 to-purple-400 mb-1">{data.name}</h2>
                ...
                <div className="font-nexa">
                  <div className="flex justify-between items-center py-4 border-b border-white/10"><strong className="text-base text-white/70">Asal</strong><p className="text-lg font-semibold text-white text-right">{data.origin}</p></div>
                  <div className="flex justify-between items-center py-4 border-b border-white/10"><strong className="text-base text-white/70">Hobi</strong><p className="text-lg font-semibold text-white text-right">{data.hobby}</p></div>
                  <div className="flex justify-between items-center py-4"><strong className="text-base text-white/70">Funfact</strong><p className="text-lg font-semibold text-white text-right">{data.funfact}</p></div>
                </div>
              </div>
...
function DecorativeDivider({ className }: { className?: string }) {
  return(
    <svg className={className} width="100%" viewBox="0 0 380 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M0 10H150" stroke="currentColor" strokeWidth="2"/>
      <path d="M380 10H230" stroke="currentColor" strokeWidth="2"/>
      <path d="M190 2.92893L195 7.92893L200 12.9289L195 17.9289L190 12.9289L185 7.92893L190 2.92893Z" stroke="currentColor" strokeWidth="2"/>
      <path d="M190 0L192.5 5L195 10L192.5 15L190 20L187.5 15L185 10L187.5 5L190 0Z" fill="currentColor" fillOpacity="0.5"/>
    </svg>
  );
}
...
function AnimatedStarfield() {
  const starLayers = useMemo(() => {
    const generateStars = (count: number) => {
      let stars = "";
      for (let i = 0; i < count; i++) { stars += `${Math.random() * 100}vw ${Math.random() * 200}vh #FFF,`; }
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .anim-star { animation: animStar linear infinite; }
        @keyframes aurora-1 {
        ...
        @keyframes aurora-2 {
            ...
            50% { transform: translate(-40px, -160px) scale(1.1); }
            75% { transform: translate(90px, -80px) scale(0.9); }
            100% { transform: translate(0, 0) scale(1); }
        }
        .animate-aurora-2 { animation: aurora-2 24s infinite ease-in-out; }
      ...
      `}</style>
      <div className="absolute top-0 left-0 w-full h-full z-0 overflow-hidden">
        {starLayers.map((layer, i) => (<div key={i} className="absolute top-0 left-0 w-full h-full anim-star" style={{ width: layer.size, height: layer.size, boxShadow: layer.shadow, animationDuration: layer.duration }}/>))}
      </div>
    </>
  );
}